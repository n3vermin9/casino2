...
btnRotate.addEventListener('click', () => {
    if (input.value > 0 && !isStarted) {
        ...
        setTimeout(() => {
            ...
            switch (true) {
                case (current <= 14 && current >= -15 || current <= -196 && current >= -225):
                  modal.innerText = 'money back'
                  localStorage.setItem('balance', balanceValue);
                  balance.innerText = localStorage.getItem('balance')
                    break;

                case (current <= -16 && current >= -45 || current <= -136 && current >= -164):
                ...
                case (current <= -46 && current >= -75 || current <= -165 && current >= -195):
                ...
                case (current <= -76 && current >= -104 || current <= -225 && current >= -255):
                ...
                case (current <= -105 && current >= -135 || current <= -256 && current >= -285 || current <= -316 && current >= -345):
...
input.addEventListener("input", function() {
    ...
    if (this.value > parseInt(balanceValue)) {
        this.value = balanceValue;
    }
});